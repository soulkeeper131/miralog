...
Шаблоните ползват `{{ brand().name }}`, `{{ brand().logo }}` и
`{{ brand().logo_full }}`; имейлите и PDF-ите четат същия източник, така че
...
Качените лога отиват в `static/uploads/` и не пипат оригиналите в `static/`,
затова „Върни оригинала“ винаги работи. При Docker тази папка трябва да е на
том, иначе качените файлове изчезват при следващия деплой.
